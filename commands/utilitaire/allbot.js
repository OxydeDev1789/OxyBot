...
module.exports = {
	...
	run: async (client, message, args, prefix, color) => {
		...
		if (client.config.owner.includes(message.author.id) || db.get(`ownermd_${client.user.id}_${message.author.id}`) === true || perm) {
			...
			if (str_filtrer.size === 0 || undefined || false || null) return message.channel.send(new Discord.MessageEmbed().setColor(color).setFooter(`${client.config.name}`).setTimestamp().setTitle("Aucun bot présents"))
